...
for fft_no = 1 : length(n_fft)
    for time_win_no = 1:2%length(time_win)
        for ms_no = 1 : length(ms)
            ...
            data_avg_txminer(fft_no,time_win_no,ms_no) = mean(data);
            data_ind_avg_txminer(fft_no,time_win_no,ms_no) = mean(data_ind);
        end
    end
end

save('data_txminer.mat','data_avg_txminer','data_ind_avg_txminer');
